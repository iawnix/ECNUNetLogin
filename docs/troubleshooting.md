...
| Login looks successful but `check` says offline                        | Portal sometimes accepts a login that the access controller then drops. Run `auth_ecnu check --json` explicitly after login when post-operation verification is required. |
